...
Retourne le résultat au format JSON avec les propriétés "subject" et "content".`
  ...
  const handleBookAppointment = async () => {
    if (!selectedDate || !selectedTime || !selectedService || !bookingForm.clientName || !bookingForm.clientEmail) {
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
          Planifiez facilement un entretien avec nos experts. Choisissez votre créneau préféré.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {selectedDate && (
              <div className="mt-4 p-4 bg-muted rounded-lg" role="status" aria-live="polite">
          ...
          <CardContent>
            {!selectedDate ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-2" role="group" aria-label="Créneaux horaires disponibles">
                  {availableSlots.map((slot) => (
                    <Button
                      ...
                      className="h-12"
                      aria-label={`Créneau de ${slot.time}, ${slot.available ? 'disponible' : 'indisponible'}`}
                ...
                {selectedTime && (
                  <div className="mt-4 p-4 bg-primary/5 border border-primary/20 rounded-lg" role="status" aria-live="polite">
          ...
          <CardContent className="space-y-6">
            <div>
              ...
              <Select value={selectedService} onValueChange={setSelectedService} required>
                <SelectTrigger id="service" aria-label="Sélectionner un type de service">
                  ...
                  {services.map((service) => (
                    <SelectItem key={service.id} value={service.id}>
                      <div className="flex justify-between items-center w-full">
                        ...
                        <span className="text-xs text-muted-foreground ml-2" aria-label={`Durée: ${service.duration} minutes`}>
            ...
            <Dialog open={isBookingOpen} onOpenChange={setIsBookingOpen}>
              <DialogTrigger asChild>
                <Button 
                  size="lg" 
                  className="w-full"
                  disabled={!selectedService || !bookingForm.clientName || !bookingForm.clientEmail || isSendingEmail}
                  aria-label="Ouvrir la confirmation du rendez-vous"
                >
                  {isSendingEmail ? (
                    <>
                      <PaperPlaneTilt className="mr-2 h-5 w-5 animate-spin" aria-hidden="true" />
                      Envoi en cours...
                    </>
                  ) : (
                    <>
              ...
              <DialogContent aria-describedby="booking-confirmation-description">
                ...
                <div className="space-y-4">
                  <div className="bg-muted p-4 rounded-lg space-y-2" role="group" aria-label="Détails du rendez-vous">
                  ...
                  <div className="space-y-2" role="group" aria-label="Informations client">
                    <div className="flex items-center gap-2">
                  ...
                  {bookingForm.notes && (
                    <div className="bg-blue-50 p-3 rounded border-l-4 border-blue-200">
                      <p className="text-sm text-blue-700"><strong>Notes :</strong> {bookingForm.notes}</p>
                    </div>
                  )}
                </div>
      ...
      {appointments && appointments.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-4" role="list" aria-label="Liste de vos rendez-vous">
              {appointments
                .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
                .map((appointment) => (
                  <div key={appointment.id} className="border border-border rounded-lg p-4" role="listitem">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Clock className="h-4 w-4 text-primary ml-2" aria-hidden="true" />
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {appointment.emailSent ? 'Email envoyé' : 'Email non envoyé'}
                          </span>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {appointment.status === 'pending' && !appointment.emailSent && (
                          <Button
                            size="sm"
                            variant="outline"
